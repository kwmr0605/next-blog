...
const CategoryCard = ({ category, subcategory, articles, icon }: CategoryCardProps) => {
  return (
    <div className="glass-card rounded-xl overflow-hidden transition-all duration-300 group hover:shadow-glass-hover">
      ...
      <div className="p-6 border-b border-subColor/20 relative">
        <div className="flex items-start justify-between mb-2">
          <div>
            <p className="text-xs text-fontSecondary uppercase mb-1">{category}</p>
            <h3 className="text-lg font-bold text-subColor group-hover:text-accentColor transition-colors drop-shadow-md">
              {subcategory}
            </h3>
          </div>
          {icon && <div className="text-accentColor opacity-60 group-hover:opacity-100 transition-opacity drop-shadow-[0_0_10px_rgba(82,190,198,0.3)]">{icon}</div>}
        </div>
      </div>
      ...
      <div className="p-6 relative">
        <p className="text-fontSecondary text-sm mb-4 line-clamp-2">
          {articles[0]?.description || articles[0]?.content.substring(0, 100) || '記事を探索してテクノロジーの最新トピックを学びましょう'}
        </p>
        ...
        <div className="space-y-3">
          {articles.slice(0, 3).map((article) => (
            ...
            >
              <svg className="w-4 h-4 mt-0.5 flex-shrink-0 group-hover/item:drop-shadow-[0_0_5px_rgba(82,190,198,0.5)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {category === 'BACK-END' && (
            <>
              <div className="w-8 h-8 rounded-full bg-glassBg backdrop-blur-glass border border-subColor/30 flex items-center justify-center hover:shadow-neon transition-shadow">
                <svg className="w-4 h-4 text-accentColor" fill="currentColor" viewBox="0 0 24 24">
              ...
              <div className="w-8 h-8 rounded-full bg-glassBg backdrop-blur-glass border border-subColor/30 flex items-center justify-center hover:shadow-neon transition-shadow">
                <svg className="w-4 h-4 text-subColor" fill="currentColor" viewBox="0 0 24 24">
