  ...
  return (
    <aside className="space-y-6">
      ...
      <div className="glass-card rounded-xl p-6 relative overflow-hidden">
        ...
        <h3 className="text-subColor font-bold text-lg mb-4 drop-shadow-md">
          Front-end Hot Topics
        </h3>
        ...
        <div className="space-y-4 relative">
          {articles.slice(0, 2).map((article) => (
            <div
              key={article.id}
              className="pb-4 border-b border-subColor/10 last:border-0 last:pb-0"
            >
              <div className="flex items-start gap-2 mb-2">
                ...
                <Link
                  ...
                  className="text-subColor font-medium text-sm hover:text-accentColor transition drop-shadow-sm"
                >
                  {article.title}
                </Link>
              </div>
              <ul className="space-y-1 ml-7 text-fontSecondary text-xs">
                <li className="flex items-start gap-2">
                  <span className="text-accentColor drop-shadow-[0_0_5px_rgba(82,190,198,0.5)]">
                    •
                  </span>
                  <span>
                    Key points:{' '}
        ...
        <ul className="space-y-3 relative">
          {articles.slice(0, 4).map((article) => (
            <li key={article.id} className="flex items-start gap-2 group">
              ...
              <Link
                href={`/articles/${article.id}`}
                className="text-fontColor text-sm hover:text-accentColor transition line-clamp-2 group-hover:drop-shadow-[0_0_8px_rgba(82,190,198,0.3)]"
              >
                {article.title}
              </Link>
            </li>
